...
| 通知推送 | `internal/notifier/dispatcher.go`（读取渠道、分发、组装卡片；`NotifyAuthorReview` 按作者逐条发送；`memberName` 按作者键查成员备注真实姓名）、`notifier.go`（`Channel`、整体 `BuildMarkdown` 与个人 `BuildAuthorMarkdown`，作者展示走 `formatAuthor`：有备注显「真名（账号）」无备注回退「git 名 &lt;账号&gt;」）、`wecom.go` / `feishu.go` / `dingtalk.go`（各平台 webhook 与加签） |
