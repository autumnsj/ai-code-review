...
| 通知推送 | `internal/notifier/dispatcher.go`（读取渠道、分发、组装卡片；`NotifyAuthorReview` 按作者逐条发送；`memberName` 按作者键查成员备注真实姓名；`SendMarkdown` 向全部启用渠道广播自定义消息）、`notifier.go`（`Channel`、整体 `BuildMarkdown` 与个人 `BuildAuthorMarkdown`，作者展示走 `formatAuthor`：有备注显「真名（账号）」无备注回退「git 名 &lt;账号&gt;」；`BuildReportMarkdown` 定时日报/周报模板：审查记录清单（仓库/PR 或分支/提交/作者/评分）+ critical/high 重点问题清单，聚焦「审了什么、发现什么问题」）、`wecom.go` / `feishu.go` / `dingtalk.go`（各平台 webhook 与加签） |
| 定时日报 / 周报 | `internal/report/report.go`（报告服务：北京时间自然日窗口 daily=[昨日0点,今日0点)/weekly=[7天前0点,今日0点)、窗口内审查记录清单 + critical/high 重点问题清单聚合（作者名复用成员备注、文件路径截短控长）、`HandleJob` 队列任务、`PayloadFor` 幂等键（定时按周期日、手动按纳秒时间戳）、配置读写 settings `report_schedules`）、`cmd/server/reportcron.go`（每分钟 ticker：命中配置的北京时间 HH:MM（周报再匹配星期）即入队 `report` 任务，同周期幂等去重、错过不补发）、`internal/server/handler_report.go`（`GET/PUT /api/admin/settings/report-schedules`、`POST /api/admin/report-schedules/preview|run`）、`internal/store/report_store.go`（`ReportRangeStats` 窗口总览聚合）；报告为模板聚合不调 LLM，无数据也发简报，推送失败只记日志不重试 |
