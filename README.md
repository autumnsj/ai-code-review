...
| 定时日报 / 周报 | `internal/report/report.go`（报告服务：北京时间自然日窗口 daily=[昨日0点,今日0点)/weekly=[7天前0点,今日0点)、按作者分组聚合「做了什么功能」（功能标题取 PR 标题或 stats.commit_subject，概述取 AI summary 首句）+ critical/high 重点问题（作者名复用成员备注、文件路径截短控长）、`HandleJob` 队列任务、`PayloadFor` 幂等键（定时按周期日、手动按纳秒时间戳）、配置读写 settings `report_schedules`）、`cmd/server/reportcron.go`（每分钟 ticker：命中配置的北京时间 HH:MM（周报再匹配星期）即入队 `report` 任务，同周期幂等去重、错过不补发）、`internal/server/handler_report.go`（`GET/PUT /api/admin/settings/report-schedules`、`POST /api/admin/report-schedules/preview|run`、`GET /api/admin/reports[/:id]` 报告记录列表/详情）、`internal/store/report_store.go`（`ReportRangeStats` 窗口总览聚合、`CreateReport` 发送时落库 `reports` 表、`ListReports`/`GetReport` 回看）；报告为模板聚合不调 LLM，无数据也发简报，推送失败只记日志不重试；发送（定时/手动）即落库、预览不落库，job_id 唯一防重试重复 |
