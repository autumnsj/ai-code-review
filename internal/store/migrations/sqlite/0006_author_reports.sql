-- 0006: 多作者归属。一次 base..head 审查可能涉及多位提交者，
-- AI 只审一次，再用 git blame 把每条 finding 归属到行作者，
-- 并在 review_author_reports 为每位参与者生成独立报告（各自评分与公开 token）。

-- findings 记录归属作者（commit author email，blame 得到；无法归属时为空串）。
ALTER TABLE findings ADD COLUMN author TEXT NOT NULL DEFAULT '';
