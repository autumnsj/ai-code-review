...
- `GET /dashboard`
- `GET /stats/leaderboard?days=30&repo_id=&limit=10` —— 多指标排行榜（完成功能数/问题数/代码量 churn/新增/删除 + 综合及四维均分），各取 Top N 一次返回；**「完成功能数」是 AI 看代码判断的功能/工作项数（`feature_count`，不数 PR、不数 commit，老数据按每条审查 1 个回退）**；`GET /stats/authors/:author` 详情含 `recent[].title` 功能标题（`domain.Review.FeatureTitle()`：PR 标题→commit 标题→分支）与行数变动
...
| 功能 | 主要代码位置 |
| --- | --- |
| 进程启动 / 装配 / 热切换 | `cmd/server/main.go`（入口）、`cmd/server/app.go`（`application` 生命周期、引导完成后进程内热切换 HTTP handler 与启动 worker/reaper/定时报告 cron）、`cmd/server/reaper.go`（定时回收卡在 pending/running 的僵尸审查：pending 超 10 分钟、running 超 35 分钟且无活跃 job 时标记 failed 并写日志）、`cmd/server/wire.go`（`reviewEnqueuer` 适配）、`cmd/server/service.go`（`adminService`：手动触发 / 任务管理 / dashboard / 定时报告配置装配） |
...
| 通知推送 | `internal/notifier/dispatcher.go`（读取渠道、分发、组装卡片；`NotifyAuthorReview` 按作者逐条发送；`memberName` 按作者键查成员备注真实姓名；`SendMarkdown` 向全部启用渠道广播自定义消息）、`notifier.go`（`Channel`、整体 `BuildMarkdown` 与个人 `BuildAuthorMarkdown`，作者展示走 `formatAuthor`：有备注显「真名（账号）」无备注回退「git 名 &lt;账号&gt;」；`BuildReportMarkdown` 定时工作日报/周报模板：按作者分组的工作内容清单（PR/commit 功能标题 + AI summary 首句功能概述，评分弱化）+ 失败审查 + critical/high 重点问题，定位是代替员工写日报）、`wecom.go` / `feishu.go` / `dingtalk.go`（各平台 webhook 与加签） |
| 定时日报 / 周报（按人出报告，用于考核） | `internal/report/report.go`（报告服务：北京时间自然日窗口 daily=[昨日0点,今日0点)/weekly=[7天前0点,今日0点)；`aggregate` 按作者小写 email 分组——功能清单取 AI 看代码判断的 `stats.features`（按 AI 标注 author 归属、未标注归审查作者；老审查无 features 时回退一条 `domain.Review.FeatureTitle()`：PR 标题 → stats.commit_subject → 目标分支），blame 到人的 critical/high 问题归入同一人；admin/空作者视为不可归属跳过；**每位有产出的成员各生成一条独立报告**（`HandleJob` 逐人落库 + 推送），失败审查与无主重点问题汇成团队级「异常提醒」，窗口无审查则发一条团队简报；`PayloadFor` 幂等键（定时按周期日、手动按纳秒时间戳）、配置读写 settings `report_schedules`）、`internal/notifier/notifier.go`（`BuildPersonalReportMarkdown` 个人报告模板：完成功能数/平均分/🔴🟠 重点问题、`BuildTeamNoticeMarkdown` 团队简报与异常提醒）、`cmd/server/reportcron.go`（每分钟 ticker：命中配置的北京时间 HH:MM（周报再匹配星期）即入队 `report` 任务，同周期幂等去重、错过不补发）、`internal/server/handler_report.go`（`GET/PUT /api/admin/settings/report-schedules`、`POST /api/admin/report-schedules/preview|run`、`GET /api/admin/reports[/:id]` 报告记录列表/详情）、`internal/store/report_store.go`（`ReportRangeStats` 窗口总览聚合、`CreateReport` 每人一条落库 `reports`（含 `author` 列，空串=团队简报/异常提醒）、`ListReports(author)`/`GetReport` 回看）；报告为模板聚合不调 LLM，无数据也发团队简报，单人落库/推送失败只记日志不影响其他人、任务整体成功不重试；发送（定时/手动）即落库、预览不落库（预览为全员拼接并标注实际每人一条），`UNIQUE(job_id, author)` 防重试重复推送 |
| 多作者拆分与归属 | `internal/analyzer/pipeline.go` 的 `persistAuthorReports`（AI 只审一次全量 base..head，按 pi-agent 的 `git blame` 归属把 findings 分桶给每位参与者，各自 `scorer.Score` 独立评分、写 `review_author_reports`、各自公开 token；blame 到区间外旧作者的 finding 只留在全量报告；AI 功能清单 `features` 按标注 author 归集、未标注归 commit 作者，写入 `feature_count`）、`author_report_store.go`（按 `(review_id,author)` 方言 upsert、按 token 查公开报告、`DeleteAuthorReports`）；迁移 `0006_author_reports.sql`（三方言，含为历史 succeeded 审查回填一条作者报告）、`0009_author_feature_count.sql`（rar 加 `feature_count`，三方言）；`internal/store/author_store.go` 看板聚合改读 `review_author_reports`（功能数 `SUM(feature_count)`，老数据按每条 1 回退）；公开端点 `GET /public/author-reports/:token`、管理端点 `GET /api/admin/reviews/:id/author-reports` |
| SSH 密钥 | `internal/sshkey/sshkey.go`（ed25519 生成、私钥解析、公钥行与 SHA256 指纹） |
| 数据访问与迁移 | `internal/store/store.go`（`Store` 聚合、方言/占位符转换）、`db.go`（打开连接，`openMySQL` 强制 `parseTime`/`multiStatements`/`utf8mb4`）、`probe.go`（连通性探测、PG/MySQL 自动建库）、`migrate.go`（嵌入式 SQL 迁移执行）、`migrations/{sqlite,postgres,mysql}/*.sql`；按领域拆分：`repo_store.go`、`credential_store.go`、`review_store.go`、`review_log_store.go`（审查过程逐行进度日志，`AppendReviewLog`/`ListReviewLogs(sinceID)` 增量拉取）、`reaper_store.go`（`ListActiveJobReviewIDs`/`ListStaleReviewIDs` 供僵尸审查回收）、`review_store.go` 的 `MarkReviewFailed`（reaper 标记超时失败）、`finding_store.go`（含 `ListFindingsByAuthor`、`author` 列）、`author_report_store.go`（按 `(review_id,author)` 方言 upsert、公开 token 查询、删除）、`job_store.go`（MySQL 用 `FOR UPDATE SKIP LOCKED` 抢占任务）、`settings_store.go`（upsert 按方言分支）、`dashboard_store.go`、`author_store.go`（作者维度聚合，数据源改读 `review_author_reports`）、`author_alias_store.go`（成员备注表 CRUD、未备注 login 发现）、`report_store.go`（定时报告窗口计数、`ListReviewsInRange` 审查清单、`ListTopFindingsInRange` 重点问题清单、`reports` 表 CRUD：`CreateReport` 带 author、`ListReports(author,limit,offset)`）；迁移 `0007_reports.sql`（reports 表，三方言）、`0008_reports_by_author.sql`（reports 加 `author` 列、唯一索引改 `(job_id, author)`、加 author 索引；MySQL 用 information_schema + PREPARE 幂等，三方言）、`0009_author_feature_count.sql`（review_author_reports 加 `feature_count`，三方言） |
...
| 报告记录 | `pages/reports/index.tsx`（已发送日报/周报列表：**成员列**（蓝色人名 Tag / 灰色「团队」）+ 成员筛选 Select、类型/统计周期/触发方式/生成时间，Modal 查看 markdown 正文）、`api/reports.ts`（`Report.author`、list 传 `author`）；侧边栏「报告记录」菜单；设置页预览 Modal 标注「实际推送每人一条」 |
| 作者维度看板 | `pages/stats/authors.tsx`（真名 / @login / 团队展示；主表含完成功能数（AI 识别 feature_count）与代码量 +/-；作者抽屉「最近完成功能」台账：功能标题/仓库/评分/行数变动/时间）、`pages/stats/leaderboard.tsx`（单页多榜柱形图：功能产出（AI 识别完成功能数/问题数）+ 代码量（总量/新增/删除）+ 各维度评分，CSS 横向柱、奖牌排名，支持时间/仓库筛选）、`api/stats.ts`（`leaderboard`） |
...
- SQLite：`0001_init.sql`、`0002_author_stats.sql`、`0003_credentials.sql`、`0004_member_and_dimensions.sql`、`0005_review_logs.sql`、`0006_author_reports.sql`、`0007_reports.sql`、`0008_reports_by_author.sql`、`0009_author_feature_count.sql`
- PostgreSQL：`0001_init.sql`、`0003_credentials.sql`、`0004_member_and_dimensions.sql`、`0005_review_logs.sql`、`0006_author_reports.sql`、`0007_reports.sql`、`0008_reports_by_author.sql`、`0009_author_feature_count.sql`（作者统计列随 0001 一并建表；新增迁移需两边同步）
- MySQL / MariaDB：`0001_init.sql`、`0003_credentials.sql`、`0004_member_and_dimensions.sql`、`0005_review_logs.sql`、`0006_author_reports.sql`、`0007_reports.sql`、`0008_reports_by_author.sql`、`0009_author_feature_count.sql`（作者统计列随 0001 一并建表；`ALTER ADD COLUMN`/外键用 information_schema + 预处理保证幂等）
...
- `0007_reports.sql`：`reports` 表（已发送日报/周报落库回看）；`0008_reports_by_author.sql`：reports 加 `author` 列、唯一索引改 `(job_id, author)`（报告以人为单位，每人一条）；`0009_author_feature_count.sql`：`review_author_reports` 加 `feature_count`（AI 看代码判断的功能数，老数据统计时按 1 回退）
